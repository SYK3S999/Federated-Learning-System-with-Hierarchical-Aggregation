            ...
            for epoch in range(local_epochs):
                for batch_idx, (data, target) in enumerate(self.trainloader):
                    ...
                    if batch_idx % 10 == 0:  # Log progress
                        logger.info(f"Epoch {epoch+1}/{local_epochs}, Batch {batch_idx}/{len(self.trainloader)}", extra={"client_id": self.client_id})
        except Exception as e:
            logger.error(f"Training failed: {str(e)}", extra={"client_id": self.client_id})
            raise
