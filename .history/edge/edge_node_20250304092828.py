...
# Custom formatter to safely handle missing edge_id
class EdgeFormatter(logging.Formatter):
    def format(self, record):
        if not hasattr(record, 'edge_id'):
            record.edge_id = 'unknown'  # Default if missing
...
handler.setFormatter(EdgeFormatter(fmt="%(asctime)s [%(levelname)s] Edge %(edge_id)s - %(message)s", datefmt="%Y-%m-%d %H:%M:%S"))
