...
class EdgeClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters: NDArrays, config: Dict[str, Scalar]) -> Tuple[NDArrays, int, Dict[str, Scalar]]:
        ...
        client_strategy = FedAvg(
            ...
            evaluate_metrics_aggregation_fn=evaluate_metrics_aggregation_fn,  # Add aggregation fn
            initial_parameters=ndarrays_to_parameters(parameters) if isinstance(parameters, list) else parameters,
        )
        max_retries = 3
        ...
        # Return required tuple: parameters, num_examples, metrics
        return self.get_parameters({}), 2000, {"aggregation_time": elapsed}

    def evaluate(self, parameters: NDArrays, config: Dict[str, Scalar]) -> Tuple[float, int, Dict[str, Scalar]]:
        self.set_parameters(parameters)
        log_resources(self.edge_id, "Pre-Evaluation")
        return 0.0, 2000, {"accuracy": 0.0}
...
if __name__ == "__main__":
    ...
    try:
        ...
        time.sleep(20)  # Delay to ensure cloud is up
