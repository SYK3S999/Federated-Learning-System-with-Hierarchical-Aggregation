...
class FlowerClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters: NDArrays, config: Dict[str, Scalar]) -> Tuple[float, int, Dict[str, Scalar]]:
        ...
        try:
            loss, accuracy = test(self.model, self.testloader, self.device)  # Use testloader
            logger.info(f"Evaluation - Loss: {loss:.4f}, Accuracy: {accuracy:.4f}", extra={"client_id": self.client_id})
            return float(loss), len(self.testloader.dataset), {"accuracy": accuracy, **pre_stats}
        except Exception as e:
            logger.error(f"Evaluation failed: {str(e)}", extra={"client_id": self.client_id})
            return float("inf"), len(self.testloader.dataset), {"accuracy": 0.0, **pre_stats}

if __name__ == "__main__":
    client_id = int(os.getenv("CLIENT_ID", 0))
    edge_id = "0" if client_id in [0, 2] else "1"  # Match depends_on
    edge_addr = f"edge{edge_id}-1:8080"  # Match docker-compose service names
