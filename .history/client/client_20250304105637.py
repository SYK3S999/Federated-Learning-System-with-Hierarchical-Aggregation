...
def log_resources(client_id, stage):
    ...
    tx_power = 0.0  # Placeholder, adjust if tracking bandwidth
    energy = float(cpu_usage) * 0.15  # Rough J/s estimate
...
def load_data(client_id):
    ...
    # Partition data (simple split for demo)
...
if __name__ == "__main__":
    ...
    edge_addr = f"edge{client_id % 2}:8080"
    max_retries = 5
    for attempt in range(max_retries):
        try:
            logger.info(f"Connecting to Edge {client_id % 2} at {edge_addr}", extra={"client_id": client_id})
            fl.client.start_numpy_client(server_address=edge_addr, client=FlowerClient(client_id))
            break
