...
import torch
import psutil
from model import Net  # Assuming model.py is copied to edge/
...
class EdgeClient(fl.client.NumPyClient):
    def __init__(self, edge_id):
        self.edge_id = edge_id
        self.model = Net()
        self.device = torch.device("cpu")  # CPU-only for pre-GPU setup
    ...
    def fit(self, parameters: NDArrays, config: Dict[str, Scalar]) -> Tuple[NDArrays, int, Dict[str, Scalar]]:
        ...
        num_examples = 15000  # From your logs
    ...
    def evaluate(self, parameters: NDArrays, config: Dict[str, Scalar]) -> Tuple[float, int, Dict[str, Scalar]]:
        ...
        return 0.0, 15000, {"accuracy": 0.0, **pre_stats}  # Placeholder, no testloader here

if __name__ == "__main__":
    edge_id = os.getenv("EDGE_ID", "0")
    cloud_addr = "172.20.0.2:8080"  # Fixed IP from docker-compose
    logger.info(f"Connecting to Cloud at {cloud_addr}", extra={"edge_id": edge_id})
