    ...
    def fit(self, parameters: NDArrays, config: Dict[str, Scalar]) -> Tuple[NDArrays, int, Dict[str, Scalar]]:
        ...
        class CustomFedAvg(FedAvg):
            ...
            def aggregate_fit(self, server_round, results, failures):
                if not results:
                    logger.warning(f"Round {server_round}: No results received", extra={"edge_id": self.edge_client.edge_id})
                    return None, {}

                # Aggregate weights (FedAvg)
                weights = super().aggregate_fit(server_round, results, failures)[0]
                if weights is None:
                    logger.warning(f"Round {server_round}: Weight aggregation failed", extra={"edge_id": self.edge_client.edge_id})
                    return None, {}

                # Aggregate metrics
                total_examples = sum(n for _, n in results)
                fit_metrics = [
                    (n, r.metrics) for (_, n), r in results if "loss" in r.metrics and "accuracy" in r.metrics
                ]
                if not fit_metrics:
                    logger.warning(f"Round {server_round}: No valid metrics received", extra={"edge_id": self.edge_client.edge_id})
                    return weights, {"loss": 0.0, "accuracy": 0.0}

                avg_loss = sum(m["loss"] * n for n, m in fit_metrics) / total_examples
                avg_accuracy = sum(m["accuracy"] * n for n, m in fit_metrics) / total_examples
                aggregated_metrics = {"loss": avg_loss, "accuracy": avg_accuracy}

                # Store aggregated metrics in edge_client
                self.edge_client.last_fit_metrics = aggregated_metrics
                logger.info(f"Round {server_round} aggregated metrics: {aggregated_metrics}", extra={"edge_id": self.edge_client.edge_id})

                return weights, aggregated_metrics
        ...
        num_examples = 15000  # Placeholder; should reflect total client examples
        ...
        if self.last_fit_metrics:
            ...
            logger.info(f"Returning aggregated client metrics to cloud: {self.last_fit_metrics}", extra={"edge_id": self.edge_id})
        else:
            logger.warning("No client fit metrics received; using fallback", extra={"edge_id": self.edge_id})
